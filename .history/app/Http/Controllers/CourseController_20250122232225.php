...
class CourseController extends Controller
{
    ...
    {
        try {
            // Validation for partner data
            $validator = Validator::make($req->all(), [
                'name' => ['required', 'string', Rule::unique('partners')],
                'description' => ['nullable', 'string'],
                'logo' => ['nullable', 'image', 'mimes:jpeg,png,jpg,gif,svg', 'max:2048'],  // Validate the logo as an image file
            ]);
            ...
            if ($req->hasFile('logo')) {
                $logo = $this->storeImage($req->file('logo'), 'uploads/partners'); // Store the logo in the 'uploads/partners' directory
                $data['logo'] = $logo;
            }
